        ...
        <div class="row">
            <div class="col-sm-12 col-md-6 col-lg-4">
                <form-select
                    id="role"
                    :title="'Role'"
                    :options="['Administrator', 'Faculty', 'Student']"
                    v-model="form.role"
                    :messager="messagers.role"
                ></form-select>
                <form-input
                    id="email"
                    :title="'Email'"
            ...
            <div class="col-sm-12 col-md-6 col-lg-4">
                ...
                <form-select
                    ...
                    :messager="messagers.department_id"
                ></form-select>
            </div>
...
export default class VueComponent extends Vue {
    ...
    private urls = {
        ["Administrator" as string]: "administrators",
        ["Faculty" as string]: "faculties",
        ["Student" as string]: "students"
    };
    messagers = {
        ["role" as string]: new ValidationMessage(),
        ...
        ["occupation_id" as string]: new ValidationMessage(),
    ...
    get departments() {
        return (this.$store.getters
            .departments as DepartmentCollection).data.map(department => {
            return {
    ...
    created() {
        ...
        this.form.department_id = String(
            (this.$store.getters.departments as DepartmentCollection).data
                .length > 0
                ? (this.$store.getters.departments as DepartmentCollection)
                      .data[0].id
                : 1
        );
    }
    submit() {
        ...
        const url = this.urls[this.form.role];
        Axios.post(url, this.form)
            .then(response => response.data)
            .then(data => {
                this.processing = false;
                toastr.success(
                    `${this.form.role} added successfully.`,
                    this.form.username
                );
                this.clear();
            })
